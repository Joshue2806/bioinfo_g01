...
echo "Filas:";
wc -l < $file; #Este comando es el de filas, se aplica sobre todos los archivos .txt de Saavedra2013 ($file)
echo "Columnas:";
head -n 1 $file | tr -d " " | tr -d "\n" | wc -c; #Igual que en la línea 11, se aplica el comando para contar columnas
done #Se acaba el bucle

#Si se desea correr este código, realizarlo en Git Bash con el comando "bash netsize_all.sh"
